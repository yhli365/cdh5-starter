...
/**
 * @link https://github.com/amutu/tdw
 * 
 * @author yhli
 * 
 */
public class ProtobufSerDe extends AbstractSerDe {
	public static final Log LOG = LogFactory.getLog(ProtobufSerDe.class
			.getName());
	...
	public void initialize(Configuration conf, Properties tbl)
		...
		String msgName = tbl.getProperty(PB_MSG_NAME);
		String outerName = tbl.getProperty(PB_OUTER_CLASS_NAME);
		...
		Class<?> tableMsgClass = ProtobufHiveUtils.loadTableMsgClass(outerName,
				msgName);
		try {
		...
		Descriptor tableMsgDescriptor = ProtobufHiveUtils
		...
		rowObjectInspector = ProtobufObjectInspectorFactory
				.getProtobufStructObjectInspector(columnNames,
	...
	static void serializeMessage(CodedOutputStream cos, Object obj,
		...
		for (int i = 0; i < fields.size(); i++) {
			serializeField(cos, soi.getStructFieldData(obj, fields.get(i)),
			...
			if (fd.isRequired()) {
				if (fd.getJavaType() != FieldDescriptor.JavaType.MESSAGE) {
					...
					PrimitiveObjectInspector poiNew = PrimitiveObjectInspectorFactory
							.getPrimitiveJavaObjectInspector(poiOld
	...
	static void serializeRepeatedField(CodedOutputStream cos, Object o,
			ListObjectInspector loi, FieldDescriptor fd)
			throws java.io.IOException {
	...
	public SerDeStats getSerDeStats() {
		// TODO no support for statistics
		return null;
	}
